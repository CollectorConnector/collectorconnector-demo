      ...
      <main style={{ marginTop: '100px', paddingBottom: '80px', maxWidth: '800px', margin: '100px auto 0', padding: '0 16px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <section style={{ background: '#09090b', border: '1px solid #27272a', borderRadius: '24px', padding: '32px', textAlign: 'center', position: 'relative' }}>
            ...
            <div style={{ position: 'relative', width: '120px', height: '120px', margin: '0 auto 24px' }}>
              <img 
                // --- FIX: Use Tier SVG logic for the profile image ---
                src={profile?.avatar_url || `/icons/tiers/${userRank || 'collector'}.svg`} 
                onError={(e) => { 
                  const target = e.target as HTMLImageElement;
                  const tierIcon = `/icons/tiers/${userRank || 'collector'}.svg`;
                  if (!target.src.includes(tierIcon)) {
                    target.src = tierIcon;
                  }
                }}
                style={{ 
                  ...
                  // Added dynamic padding for SVG icons to keep them looking sharp
                  padding: profile?.avatar_url ? '0' : '20px',
                  background: '#18181b'
                }} 
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: '#18181b', padding: '30px', borderRadius: '24px', width: '100%', maxWidth: '400px', border: '1px solid #27272a', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
                ...
                <button onClick={() => setSelectedAudience('private')} style={{ flex: 1, padding: '12px', borderRadius: '12px', fontSize: '11px', fontWeight: '900', border: '1px solid #27272a', background: selectedAudience === 'private' ? '#fff' : '#000', color: selectedAudience === '
