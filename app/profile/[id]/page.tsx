...
export default function ProfilePage() {
  ...
  // Load recent communal drops - Live Community Feed
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <main className="pt-8 pb-20 space-y-10 max-w-[720px] mx-auto px-4">
        ...
        <section className="bg-zinc-950 border border-zinc-800 rounded-2xl p-10 shadow-lg shadow-black/30">
          <div className="flex flex-col items-center text-center">
            {isOwnProfile ? (
              <label htmlFor="avatar-upload" className="relative mb-8 cursor-pointer group">
                <div className="mx-auto w-20 h-20 overflow-hidden rounded-[30%] border-4 border-zinc-700 shadow-2xl">
                  <img src={previewImage || profile.avatar_url || "/default-avatar.png"} alt="Avatar" className="w-full h-full object-cover block" />
                </div>
            ...
            {isOwnProfile && editMode ? (
              ...
              <p className="text-gray-300 text-xl mb-6 max-w-lg leading-relaxed">{profile.bio || "This collector hasn’t written a bio yet."}</p>
            )}

            {isOwnProfile && editMode ? (
              <input type="text" value={editedLocation} onChange={(e) => setEditedLocation(e.target.value)} className="text-gray-400 text-xl bg-zinc-900 border border-zinc-700 rounded px-6 py-3 text-center w-full max-w-lg mb-6" />
        ...
        {/* Live Stats with Clickable Followers/Following */}
        <section className="bg-zinc-950 border border-zinc-800 rounded-2xl p-10 shadow-lg shadow-black/30">
        ...
        <section className="bg-zinc-950 border border-zinc-800 rounded-2xl p-10 shadow-lg shadow-black/30">
          ...
          <div className="flex gap-6 overflow-x-auto pb-4 scrollbar-hide">
            ...
            {collections.map((col) => (
              <button key={col.id} onClick={() => router.push(`/collections/${col.id}/add-item`)} className="min-w-[180px] h-[120px] bg-zinc-900/70 border border-zinc-700 rounded-xl flex items-center justify-center text-xl font-medium hover:bg-zinc-800 hover:border-zinc-500 transition">
                {col.title}
              </button>
            ))}
          </div>
        ...
        <section className="bg-zinc-950 border border-zinc-800 rounded-2xl p-10 shadow-lg shadow-black/30">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
            {recentDrops.length === 0 ? (
              <div className="col-span-3 text-center py-12">
                <p className="text-zinc-500 text-xl">No drops yet — be the first to share something awesome!</p>
              </div>
            ) : (
          ...
          {recentDrops.length > 0 && <div className="text-center mt-8 text-zinc-400 text-sm">Real-time uploads from the Collector Connector community</div>}
        </section>
      </main>
    ...
    <>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 20, color: "white" }}>
          {/* Search Icon */}
          <button onClick={() => router.push("/search")} className="hover:scale-110 transition-transform p-2" aria-label="Search">
