...
export default function ProfilePage() {
  ...
  async function loadAllData() {
    try {
      ...
      // FIXED QUERY - now includes profile id
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      {/* PHOTO PREVIEW LIGHTBOX - also fixed */}
      ...
      {/* MODALS - unchanged */}
      {showAddCollection && (
        /* ... your existing showAddCollection modal ... */
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          {/* paste your original showAddCollection content here */}
        </div>
      )}

      {showEditProfile && (
        /* ... your existing showEditProfile modal ... */
      )}

      {showAddItem && (
        /* ... your existing showAddItem modal ... */
      )}

      {showEditCollection && (
        /* ... your existing showEditCollection modal ... */
      )}
