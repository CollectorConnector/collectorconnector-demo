...
export default function ProfilePage() {
  ...
  useEffect(() => {
    async function loadRecentDrops() {
      const { data } = await supabase
        ...
        .neq("status", "imported") // Added this to keep unsorted IG items out of the feed
  ...
  return (
    <div className="min-h-screen bg-black text-white" style={{ background: '#000', color: '#fff' }}>
      ...
      <main style={{ 
        ...
        <section style={{ 
          ...
          alignItems: 'center'
            ...
            {/* TOOLBAR - RE-ADDED YOUR BUTTONS + THE TWO NEW ONES */}
            {isOwnProfile && !editMode && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', justifyContent: 'center' }}>
                ...
                <button onClick={() => router.push('/curator')} style={{ background: '#fff', color: '#000', padding: '10px 20px', borderRadius: '12px', fontWeight: 'bold', fontSize: '14px', border: 'none' }}>CURATOR INBOX</button>
              </div>
            )}
        </section>
      ...
      {/* MODALS */}
      {isImportOpen && <ImportInstagramModal userId={userId} onClose={() => setIsImportOpen(false)} />}
