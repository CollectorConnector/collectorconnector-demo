...
const TIER_STYLES: Record<TierName, { bg: string; text: string; glow: string; dot: string }> = {
...
type Props = {
  tier?: string | null;       // accepts raw DB values; we’ll coerce to a valid tier
  size?: "sm" | "md" | "lg";  // visual size
  showCount?: boolean;        // e.g., “1 of 1” or “Top 5%” (simple demo)
...
export default function TierBadge({
  ...
  // default count text (feel free to adjust later)
  const effectiveCount = showCount ? (countText ?? (name === "Gold" ? "1 of 1" : "Member")) : "";
    ...
    >
      ...
      {effectiveCount && (
        <>
          ...
          />
          <span style={{ opacity: 0.9, fontWeight: 600, color: style.text }}>{effectiveCount}</span>
        </>
      )}
    </span>
  );
