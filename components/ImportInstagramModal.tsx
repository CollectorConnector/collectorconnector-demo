...
export default function ImportInstagramModal({ onClose, userId }: { onClose: () => void, userId: string }) {
  ...
  async function startImport() {
    setLoading(true);
    try {
      const res = await fetch('/api/import-ig', {
        method: 'POST',
      ...
      if (data) {
        // Bulk insert the posts into your Supabase 'items' table
      ...
      alert('Something went wrong with the import.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[1000] flex items-center justify-center p-4">
      <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-[32px] w-full max-w-md shadow-2xl">
        <h2 className="text-2xl font-black mb-2 text-center text-white">IMPORT FROM IG</h2>
